...
Relational Databases have data displayed in table form (i.e. spreadsheets from Excel). SQL is the language that is used for managing database.

2. Why do tables need a Primary Key?

Tables need a Primary Key to make sure that no two rows have the same key.
...
an intermediary table, which holds foreign keys that reference the primary key on relevant tables
